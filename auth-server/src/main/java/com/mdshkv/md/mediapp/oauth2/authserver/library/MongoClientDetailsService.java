package com.mdshkv.md.mediapp.oauth2.authserver.library;
...
public class MongoClientDetailsService implements ClientDetailsService, ClientRegistrationService {
    ...
    public ClientDetails loadClientByPackageName(String packageName) throws ClientRegistrationException {
        Query query = new Query();
        query.addCriteria(Criteria.where(MongoClientDetails.PACKAGE_NAME).is(packageName));
        MongoClientDetails clientDetails = mongoTemplate.findOne(query, MongoClientDetails.class);
        if (clientDetails == null) {
            throw new ClientRegistrationException(String.format("packageName with id %s not found", packageName));
        }
        return clientDetails;
    }
